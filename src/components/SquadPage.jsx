...
const SquadPage = () => {
  ...
  useEffect(() => {
    ...
    const unsoldP = JSON.parse(localStorage.getItem("teams")) || {};
    setUnsold(unsoldP);
  ...
  const deletePlayer = (teamName, playerName) => {
    if (
      !window.confirm(
        `Are you sure you want to remove ${playerName} from ${teamName}?`
      )
    ) {
    ...
    if (teams[teamName]) {
      const playerToDelete = teams[teamName].find(
        (player) => player.player === playerName
      );

      if (playerToDelete) {
        // Add deleted player's price back to the team's amount
        teamAmounts[teamName] =
          (teamAmounts[teamName] || 0) + playerToDelete.price;
      }
                ...
                {players.length > 0 ? (
                  <div className="grid grid-cols-3 gap-1 h-auto ">
                    {players.map((player, index) => (
                      <div
                        key={index}
                        className="bg-white  rounded shadow-lg pb-2 pt-1 flex flex-col items-center"
                      >
                        <div className="flex p-1 justify-evenly w-full border-b-2 border-black">
                          <div className=" w-auto top-2 left-2 bg-white text-black text-sm font-bold py-1 px-1 rounded-full">
                            {index + 1}
                          </div>
                          ...
                          <div className="flex gap-2 justify-around ">
                            <button
                              className="bg-blue-500  text-white px-1 py-0 rounded text-xs "
                              onClick={() =>
                            ...
                            <button
                              className="bg-red-500  text-white px-1 py-0 rounded text-xs"
                              onClick={() =>
                          ...
                          {(player.type === "bat" && (
                            <img
                              ...
                              alt="Batsman Icon"
                              className="bg-white rounded-full p-1 h-7 "
                            />
                          )) ||
                            (player.type === "wk" && (
                              <img
                                ...
                                className=" bg-white rounded-full p-1 h-7 top-0 right-0"
                              />
                            )) ||
                            (player.type === "bowl" && (
                              <img
                        ...
                        />
                        <span className="border-t-2 pt-1 px-1 font-ububtu text-xs font-bold border-[#031945] tracking-wide uppercase w-full text-center">
                          {player.player}
                        </span>
